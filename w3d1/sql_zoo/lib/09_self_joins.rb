...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      a.num, a.company, transfer_location.name, c.num, c.company
    FROM
      routes a
    JOIN
      routes b1 ON a.company = b1.company AND a.num = b1.num
    JOIN
      routes b2 ON b1.stop_id = b2.stop_id
    JOIN
      routes c ON b2.company = c.company AND c.num = b2.num
    JOIN
      stops start_location ON start_location.id = a.stop_id
    JOIN
      stops transfer_location ON transfer_location.id = b1.stop_id
    JOIN
      stops end_location ON end_location.id = c.stop_id
    WHERE
      start_location.name = 'Craiglockhart' AND end_location.name = 'Sighthill'
  SQL
end
